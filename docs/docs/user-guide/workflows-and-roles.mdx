...
## Define a Document's Workflow and Roles
...
![Diagram of the standard workflow](/images/user-guide/workflows-and-roles__uui-workflow.png)

Figure 8: Diagram of the standard workflow

### Workflows

The term workflow refers to the procession of states through which a document passes on it's way to publication. It also defines how those states are achieved. Each model type can have it's own workflow. However, the standard workflow for a mEditor model document will generally flow through the following states: draft (initial creation and the edited version), under review, approved, and published. While the document is under review, **authors**, **reviewers**, and **publishers** may communicate through the use of [comments](/user-guide/using-comments) on the document. Note that any changes to a document subsequent to initial publication will start the whole process over again. UUI documents follow the standard workflow for web site content publication, unless otherwise requested.
...
-   Add new documents. This will be indicated by an "\+ Add New" button in the upper right corner of the [document listings page](/user-guide/quick-start#viewing-documents).
...
-   Respond to comments issued by the reviewer(s). This is done via the [comments](/user-guide/using-comments) feature.

#### Reviewer

If you are a reviewer, you may:

-   Make comments on documents that are "Under Review". Once notified, via an email prompted by an author indicating review readiness, you will need to conducted an editorial review the document, making comments using the [comments](/user-guide/using-comments) feature as needed.
