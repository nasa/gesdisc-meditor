...
export class ModelState {
    ...
    getAllModels(
        { patchState, getState }: StateContext<ModelStateModel>,
        { payload }: actions.GetAllModels
    ) {
        ...
        if (useCache) {
            ...
            return this.service.listModels().pipe(tap(getAllModelsCallback))
        }
    }

    @Action(actions.GetModel)
    getModel(
        { patchState, getState, dispatch }: StateContext<ModelStateModel>,
        { payload }: actions.GetModel
    ) {
        const model: any = getState().currentModel
        const useCache: boolean =
            model && model.name === payload.name && !payload.reload

        console.log('get model ', payload)
    ...
    getModelDocuments({ patchState, getState }: StateContext<ModelStateModel>) {
        ...
        return this.service.listDocuments(getState().currentModel.name).pipe(
            tap((currentModelDocuments: DocCatalogEntry[]) => {
                currentModelDocuments.sort(function(a, b) {
                    if (a['x-meditor'].modifiedOn > b['x-meditor'].modifiedOn) {
                        return -1
                    }
                    if (a['x-meditor'].modifiedOn < b['x-meditor'].modifiedOn) {
                        return 1
