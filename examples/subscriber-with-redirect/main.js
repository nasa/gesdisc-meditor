...
const CLIENT_ID = "meditor_example_subscriber_with_redirect"; // change this to uniquely identify the subscriber
const SUBSCRIBED_CHANNELS = ["meditor-Howto"]; // one or more channels to subscribe to
...
const USE_MANUAL_ACKNOWLEDGEMENTS = true;
...
log.notice(
  `Attempting to connect client (${CLIENT_ID}) to NATS (Cluster: ${CLUSTER_ID}, Server: ${SERVER})`
);
...
function handleMessage(message) {
  ...
  try {
    ...
    let exampleAcknowledgement = {
      ...
      redirectToUrl: true,
      message: "Successfully published Howto article",
      statusCode: "200",
    };
    ...
    stan.publish(
      "meditor-Acknowledge",
      JSON.stringify(exampleAcknowledgement),
      () => {
