...
@Component({
    ...
    template: `
        <button
            color="primary"
            (click)="goToSearch.emit(model)"
            fxFlex="120px"
            mat-raised-button
            style="line-height: inherit; padding: 10px;"
        >
            <i
                class="icon-badge icon-badge-lg fa {{
                    model.icon?.name
                }} align-middle"
            ...
            <div style="font-weight: bold;">
                {{ model.name | bcEllipsis: 10 }}
            </div>
            <div>
                ({{
                    model['x-meditor'] && model['x-meditor'].count
