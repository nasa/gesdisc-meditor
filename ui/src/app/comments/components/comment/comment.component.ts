...
import { Comment, User } from '../../../service/model/models'
...
export class CommentComponent implements OnInit {
    ...
    set comments(comments: Comment[]) {
        ...
        if (this._versionFilter)
            this.extComments = this.extComments.filter(c => {
    ...
    openReplyForm(_id: string) {
        let comment = _.find(this.extComments, function(comment) {
            return comment._id == _id
        })
        if (comment && comment.parentId == 'root') {
            let commentThread = this.commentThreads.find(i => {
                return i._parentId == _id
            })
            if (commentThread && commentThread.extComments.length > 0) {
                commentThread.openReplyForm(_id)
            } else {
                comment.replyTo = true
            }
        } else {
    ...
    openEditForm(_id: string, text: string) {
        _.find(this.extComments, function(comment) {
            return comment._id == _id
        }).edit = true
