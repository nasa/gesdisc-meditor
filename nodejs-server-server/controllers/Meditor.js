...
nats
  .subscribeToChannel("meditor-Acknowledge")
  .on("message", handlePublicationAcknowledgements);
...
function handleResponse(response, res, defaultStatus, defaultMessage) {
  ...
  if (_.isObject(res) && _.get(res, "message", null) !== null)
    result = res.message;
...
function getDocumentAggregationQuery(meta) {
        ...
        $addFields: {
          banTransitions: {
            $eq: [
              {
                $cond: {
                  if: { $in: ["$x-meditor.state", meta.exclusiveStates] },
                  then: meta.user.uid,
                  else: "",
                },
              },
  ...
  if ("title" in meta.params)
    searchQuery[meta.titleProperty] = meta.params.title;
...
async function saveDocument(client, request, document, model) {
  ...
  let state =
    document["x-meditor"].states[document["x-meditor"].states.length - 1]
      .target;
...
async function deleteDocument(client, model, title, user) {
  if (!model || !title)
    throw new Error("Please provide a model and document title");
...
module.exports.setup = async function (request, response) {
  ...
  try {
    ...
    let models = JSON.parse(
      await readFile(__dirname + "/../db-seed/models.json")
    );
  ...
  try {
    ...
    try {
      ...
      request.swagger.params.title.value =
        request.swagger.params.newTitle.value;
    ...
    .then(function () {
      ...
      return that.dbo
        ...
        .map(function (doc) {
          ...
          res["x-meditor"] = _.pickBy(doc["x-meditor"], function (value, key) {
...
function getCommentsforDoc(params) {
  return new Promise(function (resolve, reject) {
    MongoClient.connect(MongoUrl, function (err, db) {
      ...
      dbo
        .collection("Comments")
        .find({ $and: [{ documentId: params.title }, { model: params.model }] })
        .toArray(function (err, res) {
          if (err) {
            console.log(err);
            throw err;
